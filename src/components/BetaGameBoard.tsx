...
import { emptyAttributes, mergeAttributes } from "@/types/beta";
...
export default function BetaGameBoard({
  sessionId,
  timeLimitSeconds,
  cardId,
  cardNames,
  ...
  const [attributes, setAttributes] = useState<CardAttributes>(emptyAttributes());
  ...
  const fetchSummary = useCallback(async () => {
    try {
      const res = await fetch("/api/question-beta", {
        method: "POST",
  ...
  async function handleAsk(question: string) {
    ...
    try {
      ...
      if (data.attrs && Object.keys(data.attrs).length > 0) {
        setAttributes((prev) => mergeAttributes(prev, data.attrs));
      }
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-[var(--border)] shrink-0">
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs text-[var(--accent)] font-medium">BETA</span>
      ...
      {/* Card visual + Q&A side by side on desktop, stacked on mobile */}
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden min-h-0">
        {/* Card visual */}
        <div className="flex justify-center items-start p-3 md:p-4 shrink-0">
          <CardVisual
            attributes={attributes}
            revealedName={phase === "revealed" ? reveal?.card.name : undefined}
            revealedArt={phase === "revealed" ? reveal?.card.image_uri_art_crop || undefined : undefined}
          />
        </div>
        ...
        <div ref={qaContainerRef} className="flex-1 overflow-y-auto px-4 py-2 min-h-0">
          {questions.length === 0 && (
            <div className="text-[var(--text-secondary)] text-center py-3">
              ...
              <p className="text-sm mt-1">Watch the card fill in as you discover attributes!</p>
            </div>
          )}
